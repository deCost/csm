	...
	public class Utilities
	{
		public static void GetStudentsTotalPoints (ref List<StudentSchedule> students)
		{
			...
			students.ForEach (delegate(StudentSchedule s) {
				List<StudentSchedule> loopList = tmpstudents.FindAll (s2 => s2.UserID == s.UserID && s2.SchedDate < DateTime.Now);
				decimal totalSum = 0;

				int total = loopList.Count;
				DateTime lastDate = total > 0 ? loopList.Max (s2 => s2.SchedDate) : DateTime.Now;
				//decimal totalPoint = tmpstudents.FindAll(s2 => s2.UserID == s.UserID).Sum(s2 => s2.Points);
				
				if (total > 0) {
					//totalSum += totalPoint;
					totalSum += (decimal)(DateTime.Now.Subtract (lastDate).TotalDays * -5);
